...
function vector.distance2( x1, y1, x2, y2 )
	local dx = x1 - x2
	local dy = y1 - y2
	return dx*dx + dy*dy
end
...
function vector.snap( x, y, gx, gy )
...
function vector.approach( x1, y1, x2, y2, step )
	local dx = x2 - x1
	local dy = y2 - y1
...
function vector.projectOn( x1, y1, x2, y2 )
...
function vector.mirrorOn( x1, y1, x2, y2 )
...
function vector.trim( x, y, maxLength )
